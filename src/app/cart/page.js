...
const SwipeButton = ({ onConfirm, disabled }) => {
  ...
  useEffect(() => {
    const handleMove = (e) => {
      ...
      const containerRect = containerRef.current.getBoundingClientRect();
      const thumbWidth = thumbRef.current.offsetWidth;
      const maxLeft = containerRect.width - thumbWidth - 12; // 6px padding on sides
  ...
  return (
    <div className={`swipe-wrapper ${disabled ? 'disabled' : ''}`}>
      <div className={`swipe-tooltip ${isDragging ? 'visible' : ''}`}>
        Return here to complete payment 💸
      ...
      <div className="swipe-container" ref={containerRef}>
        ...
        >
          {isComplete ? (
            ...
            <svg className="wa-icon-thumb" viewBox="0 0 24 24" width="26" height="26" fill="currentColor">
    ...
    <div className="cart-page">
      ...
      {!isStoreOpen ? (
        ...
        <div className="wa-order-wrap">
          <SwipeButton onConfirm={handleWhatsAppOrder} disabled={isNavigating} />
          <p className="wa-order-hint">Slide to send order details via WhatsApp. No payment needed yet.</p>
        </div>
      ) : (
